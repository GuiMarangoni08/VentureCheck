...
create table if not exists vc_feedback (
  ...
  session_id  text,
  plan        text,
  question    text,        -- 'analise' | 'produto' | 'profundidade'
  rating      integer,     -- 1-5
  comment     text,
  created_at  timestamptz default now()
);
...
-- Políticas anon
create policy "anon insert sessions"  on vc_sessions for insert to anon with check (true);
create policy "anon select sessions"  on vc_sessions for select to anon using (true);
create policy "anon update sessions"  on vc_sessions for update to anon using (true);

create policy "anon insert drafts"    on vc_drafts for insert to anon with check (true);
create policy "anon select drafts"    on vc_drafts for select to anon using (true);
create policy "anon update drafts"    on vc_drafts for update to anon using (true);

create policy "anon insert feedback"  on vc_feedback for insert to anon with check (true);
create policy "anon select feedback"  on vc_feedback for select to anon using (true);
